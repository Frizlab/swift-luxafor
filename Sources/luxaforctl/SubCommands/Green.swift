...
struct Green : AsyncParsableCommand {
	
	@OptionGroup
	var rootOptions: Luxaforctl.Options
	
	@Option(help: "Percentage of brightness of the green. Must be between 0 and 100 (both included).")
	...
	func run() async throws {
		for luxafor in try Luxaforctl.getLuxafors(for: rootOptions) {
			try await luxafor.setColor(on: leds, red: 0x00, green: UInt8(0xff * brightness/100), blue: 0x00, fadeDuration: fadeDuration)
		}
	}
